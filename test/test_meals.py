...
class MealViewsTest(TestCase):
  """Defines the test suite for the meal api views."""

  def setUp(self):
    """Define test client and other test variables"""
  self.client = APIClient()
  self.sandwich = Food.objects.create(name="sandwich", calories=400)
  self.pizza = Food.objects.create(name="pizza", calories=700)
  ...
  self.lunch.foods.add(self.pizza)
  ...
  def test_gets_individual_meal(self):
    ...
    self.assertEqual(meal_response["foods"][0], self.pizza.name)
    self.assertEqual(meal_response["foods"][0]["calories"], self.pizza.calories)
  
  def test_api_can_add_food_to_a_meal(self):
    """Test the api can add a food to a meal."""
    response = self.client.post(f"/api/v1/meals/{self.dinner.id}/foods/{self.sandwich.id}")
    self.assertEqual(response.data["message"], "Successfully added {self.sandwich.name} to {self.dinner.name}")

  def test_api_can_delete_food_from_a_meal(self):
    """Test the api can delete a food from a meal."""
    response = self.client.delete(f"/api/v1/meals/{self.dinner.id}/foods/{self.sandwich.id}")

    self.assertEqual(response.data["message"], "Successfully removed {self.sandwich.name} to {self.dinner.name}")